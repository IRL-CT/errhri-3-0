...
Participants submit **window-level predictions** (one row per window per clip), including predicted probability scores, and declare the `window_size` and `slide` they used. The organizer uses those parameters to aggregate the frame-level ground truth — since all frames within a clip share the same label, the aggregated GT is always identical to the video label.

Metrics are reported at two levels: **window** and **video**. The two tracks have different primary metrics reflecting their different temporal structures — see the Metrics section below.
...
Participants choose how to process their recordings and which window parameters to use — that is part of their method. When submitting, they declare `window_size` and `slide`. The organizer then applies track-specific aggregation:

**Track 1 (BAD):**
1. Takes the participant's window-level predictions directly for **window-level metrics**
2. Applies **majority vote** across all windows per clip for the **video-level prediction** (ties resolved in favour of Failure)
3. Joins the video-level ground-truth label (constant within each clip) onto both levels

**Track 2 (Bad Idea):**
1. Takes the participant's window-level predictions directly for **window-level metrics**
2. Takes the **maximum `y_prob_1`** across all windows per clip as the **video-level score** for AUC computation
3. Joins the video-level ground-truth label onto both levels
...
The two tracks have different primary metrics, reflecting their different temporal structures.

### Track 1 (BAD) — Primary Metrics (video level, both used for ranking)

| Metric | Why |
|---|---|
| **Macro F1** | Averages F1 equally across both classes, penalising poor performance on either |
| **Balanced Accuracy** | Mean recall per class; robust to imbalance — especially important given 40 failure vs. 6 control videos |
...
### Track 2 (Bad Idea) — Primary Metric (video level)

| Metric | Why |
|---|---|
| **AUC-ROC** (max aggregation) | The video-level score is the maximum `y_prob_1` across all windows. This rewards models that fire confidently on the relevant anticipatory signal even if it appears only briefly — matching the biological reality that anticipatory reactions may be concentrated in just one or two windows near the end of the clip. AUC is threshold-free and robust to the class balance of this dataset. |

Teams are ranked by AUC. **This metric must be reported in the submitted paper.**

### Secondary Metrics (both tracks, reported at window and video level)

| Metric | Notes |
|---|---|
| Macro F1 | Video level; reported as secondary for Track 2 |
| Balanced Accuracy | Video level; reported as secondary for Track 2 |
| F1 (positive class) | Failure / Poorly |
| F1 (negative class) | Control / Well |
| Precision & Recall | For the positive class |
| Accuracy | |
| AUC-ROC | Reported as secondary for Track 1 |
...
> **Track 2 note:** Bad Idea clips are very short (~1.95 s, ~58 frames). Temporal metrics are reported for completeness, but their interpretation differs from Track 1: because clips are brief and the task captures a participant's instantaneous anticipatory state rather than an unfolding event, "earliest detection time" and FNR reflect model consistency across windows rather than a meaningful temporal detection trajectory. Participants should not over-optimise for these metrics on Track 2.
...
Teams are ranked **separately per track**:
- **Track 1**: ranked by video-level macro F1, balanced accuracy as tiebreaker
- **Track 2**: ranked by video-level AUC-ROC (max aggregation)
